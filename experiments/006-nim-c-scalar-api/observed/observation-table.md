...
| C11 callerでの`NIM_EXTERNC` → `N_LIB_EXPORT` → `N_CDECL` | `__attribute__((visibility("default"))) int asmati_macro_probe(int value);` | Cでは`NIM_EXTERNC`が空になり、定義鎖全体がdefault visibility付きのC関数定義へ展開された |
